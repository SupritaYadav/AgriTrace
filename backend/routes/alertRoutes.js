...
import { listAlerts, getAlertById, acknowledgeAlert, resolveAlert } from "../services/alertService.js";
import { success, error } from "../utils/apiResponse.js";

const router = express.Router();

// List alerts with filters & pagination
router.get("/", getCurrentUser, requireRole(Role.ADMIN, Role.FARMER, Role.TRANSPORTER, Role.WAREHOUSE), async (req, res) => {
  try {
    const { status, severity, shipmentId, deviceId, page, limit } = req.query;
    const result = await listAlerts({ status, severity, shipmentId, deviceId, page: Number(page), limit: Number(limit) });
...
// Get single alert
router.get("/:alertId", getCurrentUser, requireRole(Role.ADMIN, Role.FARMER, Role.TRANSPORTER, Role.WAREHOUSE), async (req, res) => {
  try {
